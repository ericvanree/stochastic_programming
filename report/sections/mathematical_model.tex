...
\begin{align}
  Q(x, s) = \min_{S,W,I,O} \;
    ...
    + \beta_2 \!\!\sum_{\substack{p,p' \in P \\ p' \neq p}}\!\! I_{pp's} \nonumber \\
...
keeping $|S|$ moderate, for instance through SAA, in which $|S|$ scenarios are drawn from the duration distribution and
...
Big-$M$ coefficients in equations~2, 5, 6, 11, 15, 23, 26 and 27. This makes it difficult for a solver to find a lower bound, which leaves the optimality gap wide.
...
\textbf{SB1} enforces a specialty ordering across sessions: sessions
...
Despite these measures, when solving for step 3, the model does not reach optimality in resonable time (within an hour), therefor, a 300-second time limit 
is applied to balance solution quality with run time.