...
consisting of multiple operating rooms~(ORs). At the start of each week, all 
...
The objective is to minimise the weighted sum of three cost components: the total patient waiting time (positive difference between 
actual and planned start time), the total OR idle time (slack 
between consecutive surgeries beyond the mandatory changeover), and the 
total overtime (time by which the last surgery in an OR exceeds 
$t_{\text{close}}$). 

Because $D_p$ is continuous and its realisation is only observed intraoperatively, 
an exact deterministic formulation is insufficient. We therefore adopt a 
two-stage stochastic programming framework with sample average 
approximation~(SAA)~\cite{Kleywegt}: First-stage decisions (assignment, 
sequencing, timing) are made before uncertainty is revealed, while second-stage 
costs (waiting, idle, overtime) are evaluated over a finite scenario set. The goal is to find a schedule that minimises 
