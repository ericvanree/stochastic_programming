...
Tables~\ref{tab:bp_seeds_quality} and~\ref{tab:bp_seeds_quotas} summarise
the nine blueprint solves over seeds $\theta \in \{42, \ldots, 50\}$ using a
larger SAA instance ($|R|=5$, $|S|=20$, LHS, $\beta_5 = 1$).
All runs hit the 600-second time limit.
The large residual gaps ($\approx$79--84\,\%) sit entirely in the operational
cost component; the blueprint capacities $N_{gh}$ are at the proven lower
bound in every run, confirming that the slot-cost term is reliably identified
...
\paragraph{Cross-seed observations.}
Three regularities emerge from Table~\ref{tab:bp_seeds_quotas}.
First, the \emph{S~(Short) total is the most stable dimension}: seven of
nine seeds produce $\sum_h N_{Sh} \in \{16, 17\}$, and the modal value
is~16 (seeds 42, 43, 46, 49).
This matches the largest short-case count observed across the five sampled
waiting lists, confirming that the $\beta_5$~penalty drives exact worst-case
provisioning without slack.
Second, the \emph{L~(Long) allocation is structurally invariant when present}:
all five seeds where $\sum_h N_{Lh} \geq 1$ concentrate every long-case
slot in session~0 ($N_{L1} = N_{L2} = 0$ without exception).
This is not an artefact of the symmetry-breaking ordering constraint; it is
an emergent optimisation outcome, indicating that reserving one dedicated
session for high-variance procedures is genuinely optimal for this case mix.
Third, the \emph{M~(Medium) subgroup is the primary source of variability}:
$\sum_h N_{Mh}$ ranges from~1 to~4 across seeds, reflecting the sensitivity
of medium-case provisioning to which particular waiting lists are sampled.

\paragraph{Managerial implications.}
The cross-seed pattern supports three planning recommendations.
\emph{(i)~Designate one session as the long-case anchor.}
Concentrating the single long procedure in session~0 isolates duration
uncertainty and prevents an overrun from cascading into subsequent sessions;
the optimiser selects this structure in every seed that provisions a long slot,
regardless of the particular waiting lists drawn.
\emph{(ii)~Provision at least one long-case slot by default.}
Long surgery types represent only three of 68 types, yet they appear in five
of the nine sampled waiting lists ($\approx$55\,\%), so a blueprint with
$\sum_h N_{Lh} = 0$ will be infeasible roughly half the time under the
observed ORT mix.
Risk-neutral provisioning sets $N_{L0} = 1$.
\emph{(iii)~Distribute medium-case slots uniformly across sessions.}
Medium-duration procedures (mean $\approx$126\,min) can be accommodated in
any session without materially increasing overtime risk; a uniform allocation
$N_{Mh} = 1$ for all $h$ avoids concentrating scheduling uncertainty in a
single session and keeps all sessions comparably loaded.

\subsection{Recommended Blueprint}
\label{sec:bp_robust}

Aggregating the stability evidence, solution quality, and the managerial
principles above, seed~42 is recommended as the robust blueprint setting.
It achieves the lowest in-sample objective (95.7) among the nine runs and
the second-lowest optimality gap (80.2\,\%), and produces quotas that agree
with the modal pattern across all seeds: $\sum_h N_{Sh} = 16$ (modal S
total, shared by seeds 42, 43, 46, 49), $\sum_h N_{Mh} = 3$ (modal M
total, shared by seeds 42, 45, 46, 49), and $\sum_h N_{Lh} = 1$ (L
provision maintained, present in five seeds).
The session-level allocation---$S{:}[6,5,5]$, $M{:}[1,1,1]$,
$L{:}[1,0,0]$---distributes short-case and medium-case capacity
symmetrically across sessions while isolating the long case in session~0,
consistent with the invariant observed in every seed that provisions a
long-case slot.
This blueprint is adopted as the reference policy in the operational cost
comparison of Section~\ref{sec:bp_apply}.
