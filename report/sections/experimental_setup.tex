...
scheduling the extreme duration scenarios---procedures running substantially
longer or shorter than expected---drive overtime, idling, and waiting costs, yet
random sampling assigns them low probability, requiring many scenarios for
reliable estimates.

To enrich the set with such tail events we apply the mean-shifting strategy
...
\end{equation}
%
where $f(\cdot \mid \mu, \sigma_\tau)$ is the nominal lognormal density,
...
Consequently the enriched tail scenarios---precisely those driving overtime and
cascade waiting---carry \emph{small} weights, so the effective sample size
$N_{\text{eff}} = 1 / \sum_s \pi_s^2$ is below $N$, and for small $N$ IS may need
\emph{more} scenarios than RS for a stable estimate. The compensating benefit is
...
Step~1, the sequencing). Step~3 uses four patient sets (waiting lists~4, 7, 10,
