...
To regard the advance and allocation steps, the sequential approach allows modelling the problem in a way that does not require pre-defining a total number of slots per session. In addition, the problem is dealt with as a block scheduling problem, in which blocks are separately sequenced to simplify the problem at hand. 
...
determines a suitable number of scenarios~$N$ under a chosen sampling method and an evaluation
...
a continuous distribution whose infinite scenario space is intractable,
...
structure keeps each subproblem relatively small.