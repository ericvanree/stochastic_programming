...
A key difficulty in OT scheduling is that surgery durations are uncertain \cite{CARDOEN2010921}. Deterministic schedules based only on expected durations often perform poorly in practice, since deviations in realized procedure times propagate throughout the day and lead to patient waiting, downstream idle time, and overtime. Literature therefore increasingly models OR scheduling as an optimization problem under uncertainty.
